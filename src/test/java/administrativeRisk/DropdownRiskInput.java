...
public class DropdownRiskInput extends BaseClass {
	@Test
	public void riskInputDropdownInAdministrativeRisk() {
	...
	extentTest.info("Successfully clicked on Risk Assessment on left navigation bar in Administrative risks page");
		...
		js.executeScript("arguments[0].scrollIntoView(true)", basePg.getRiskInputLabel());
		explicit.until(ExpectedConditions.presenceOfElementLocated(By.id("RiskInput_chosen")));
		basePg.getRiskInputDropdown().click();
		data.checkDropDownListfromUIAndExcel(driver, "Risk Input", "Administrative");
	}
